...
while True:
    ...
    print("received message: " ,  len(data), round(struct.unpack('I', data[-6:-2])[0], 2) * 0.000001, struct.unpack('cc', data[-2:]))
"""
...
all_points = np.array([]).reshape(-1, 5)
c = 0
while True:
    ready_read, _, _ = select.select([sock1, sock2], [], [], None)
    for ready in ready_read:
        ...
        if addr[1] == 2368:
            ...
            time_in_sec_from_hour = unpacked[-1] * 1 ^ -6
